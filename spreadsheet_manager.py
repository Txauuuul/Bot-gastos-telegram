...
class SpreadsheetManager:
    ...
    def __init__(self, use_google_drive: bool = False):
        ...
        self._creds = None
        self._wb_cache = None  # Caché del workbook en memoria
    ...
    def _refrescar_token_si_necesario(self) -> bool:
        """Refresca el token de Google Drive si ha expirado."""
        try:
    ...
    def _crear_excel_base(self) -> None:
        ...
        self._wb_cache = wb  # Cachear el workbook recién creado
        logger.info(f"📄 Archivo {self.LOCAL_FILE} creado")

    def _obtener_workbook(self):
        """Obtiene el workbook desde caché o disco."""
        ...
        self._wb_cache = openpyxl.load_workbook(self.LOCAL_FILE)
        return self._wb_cache

    def _guardar_workbook(self):
        """Guarda el workbook cacheado a disco."""
        if self._wb_cache is not None:
            self._wb_cache.save(self.LOCAL_FILE)

    def _invalidar_cache(self):
        """Fuerza recargar el workbook del disco."""
        self._wb_cache = None

    def agregar_gasto(self, gasto: Dict) -> Tuple[bool, bool]:
        """
        ...
        Returns:
            Tupla (guardado_ok, sync_ok)
        """
        try:
            if not os.path.exists(self.LOCAL_FILE):
                self._crear_excel_base()

            wb = self._obtener_workbook()
            ws = wb.active
    ...
    def _descargar_excel_del_google_drive(self) -> bool:
        ...
        try:
            ...
            self._invalidar_cache()  # Forzar recarga del nuevo archivo
            return True
    ...
    def _sincronizar_google_drive(self) -> bool:
        """Carga el Excel a Google Drive INMEDIATAMENTE."""
        try:
            if not self.drive_service:
    ...
    def eliminar_gasto(self, concepto: str) -> Dict:
        ...
        try:
            ...
            self._guardar_workbook()
            self._invalidar_cache()  # Invalidar tras borrar filas
    ...
    def deshacer_ultimo_gasto(self) -> Dict:
        ...
        try:
            ...
            guardado_ok = resultado[0] if isinstance(resultado, tuple) else resultado
            if guardado_ok:
                self._guardar_historial_borrados(historial)
                return {
                    "exito": True,
    ...
    def duplicar_gasto(self, concepto: str) -> Dict:
        ...
        try:
            ...
            resultado = self.agregar_gasto(nuevo_gasto)
            guardado_ok = resultado[0] if isinstance(resultado, tuple) else resultado
            if guardado_ok:
                return {
                    "exito": True,
